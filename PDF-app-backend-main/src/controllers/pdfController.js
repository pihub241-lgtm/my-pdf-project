...
async function addWatermark(req, res, next) {
  ...
  const watermarkText =
    String(req.body.watermarkText || req.body.text || '').trim() || 'elexico';
  const opacityRaw = Number(req.body.watermarkOpacity);
  const watermarkOpacity = Number.isFinite(opacityRaw)
    ? Math.min(Math.max(opacityRaw, 0.2), 0.3)
    : 0.25;
  const watermarkPageRaw = String(req.body.watermarkPage || '').trim();
  ...
  try {
    ...
    if (watermarkPageRaw) {
      const pageNumber = Number(watermarkPageRaw);
      if (!Number.isInteger(pageNumber) || pageNumber < 1 || pageNumber > totalPages) {
        throw createHttpError(
          400,
          `watermarkPage must be between 1 and ${totalPages}.`
        );
      }
      targetPageIndexes = [pageNumber - 1];
    }
